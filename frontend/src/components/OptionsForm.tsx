...
function OptionsForm ({inputOptions, selectOptions, onOptionsChange, optionsState, dependencyConfig}: Props) {
    ...
    <>
        <div className="blue-glass flex flex-col justify-center items-center">
            ...
            <div className={`${collapse ? "gap-0" : "gap-3"} flex flex-col justify-center items-center transition-all ease-in duration-1000`}>
                ...
                {Object.entries(inputOptions).map(([key, value]) => {
                    ...
                    return(
                        <div className={`${collapse ? "invisible opacity-0 h-0" : "opacity-100 h-12"} items-center transition-all ease-in duration-1000 flex flex-col`}>
                            <label>{key}</label>
